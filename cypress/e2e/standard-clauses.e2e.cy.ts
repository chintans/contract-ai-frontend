...
describe('Standard Clauses E2E', () => {
  ...
  it('should show empty state if no clauses exist', () => {
    cy.intercept('GET', `${apiUrl}/api/standard-clauses`, { body: [] }).as('getClauses');
    cy.reload();
    cy.wait('@getClauses');
  ...
  it('should add a new clause and see it in the list', () => {
    cy.intercept('POST', `${apiUrl}/api/standard-clauses`, req => {
      req.reply({
        statusCode: 201,
        body: {
          id: 2,
    ...
    cy.intercept('GET', `${apiUrl}/api/standard-clauses`, { body: [
      ...initialClauses,
      {
        id: 2,
        name: 'Test Clause',
  ...
  it('should view details for a clause', () => {
    cy.intercept('GET', `${apiUrl}/api/standard-clauses/1`, { body: initialClauses[0] }).as('getClauseDetail');
  ...
  it('should delete a clause and remove it from the list', () => {
    ...
    cy.intercept('DELETE', `${apiUrl}/api/standard-clauses/1`, { statusCode: 204 }).as('deleteClause');
    cy.intercept('GET', `${apiUrl}/api/standard-clauses`, { body: [] }).as('getClausesAfterDelete');
  ...
  it('should show error alert if an error occurs', () => {
    cy.intercept('GET', `${apiUrl}/api/standard-clauses`, { statusCode: 500, body: { message: 'Server error' } }).as('getClausesError');
    cy.reload();
  ...
  it('should show loading state', () => {
    cy.intercept('GET', `${apiUrl}/api/standard-clauses`, (req) => {
      req.on('response', (res) => {
        res.setDelay(1000);
      });
